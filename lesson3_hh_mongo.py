...
df = df[['name', 'min_salary', 'max_salary', 'currency', 'link']]
#print(df)

#реализовать функцию, которая будет добавлять только новые вакансии/продукты в вашу базу.
def insert_vacancy(vacancies_list):
    for vacancy in vacancies_list:
        if hh.find_one({'link': vacancy['link']}):
            print(f"Вакансия по ссылке {vacancy['link']} уже существует в базе данных")
        else:
            hh.insert_one(vacancy)

insert_vacancy(vacancies_list)
...
def g_find_salary(salary):
    for doc in hh.find({'$or': [{'max_salary': {'$gt': salary}},
                                {'min_salary': {'$gt': salary}}]}):
        pprint(doc)

g_find_salary(150000)